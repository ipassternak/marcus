...
class Token {
  ...
  constructor(index, type, ...flanks) {
    this.#type.push(type);
    this.index = index;
    this.#type.push(...flanks.filter((flank) => flank));
  }
...
const evalPreformatted = (acc, scopes) => {
  const res = [];
...
const checkNested = (scope) => {
  ...
  for (const delimeter of DELIMETERS) {
    ...
    if (right) {
      throw new Error(
        `nested ${delimeter} token found at index: ${right.index}`,
      );
    }
  }
};
...
const pipeline = [evalPreformatted, evalParagraph, evalDelimeters];

const sanitize = (src) => src.trim().replace(/\n{3,}/g, '\n\n');
