...
const { TYPES, MARKING, HTML_MAP } = require('./constants.js');
...
const compile = (map) => {
  ...
  return (src) => {
    let offset = 0;
    const res = [open];
    const { tokens, processed } = parse(src);
    for (const token of tokens) {
      const compiler = compilers[token.type];
      const compiled = compiler(processed, token, offset);
      offset = token.index + MARKING[token.type].length;
      res.push(compiled);
    }
    res.push(processed.slice(offset, processed.length), close);
    return res.join('');
  };
};
